...
//
//  Created by Анастасия Стрекалова on 24.03.2020.
...
class GameScene: SKScene {
    ...
    let fusePosition = CGPoint(x: 78, y: 64)
    let enemyCreationXPosition = 64...960
    let enemyCreationYPosition = -128
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            if node.name == "enemy" {
                score += 1
            } else if node.name == "enemySpeedy" {
                score += 5
            }
            if node.name == "enemy" || node.name == "enemySpeedy" {
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if enemyType == 0 {
            ...
            enemy = SKSpriteNode(imageNamed: "penguinSpeedy")
            run(SKAction.playSoundFileNamed("launch.caf", waitForCompletion: false))
            enemy.name = "enemySpeedy"
        } else {
        ...
        let randomPosition = CGPoint(x: Int.random(in: enemyCreationXPosition), y: enemyCreationYPosition)
